...
/**
 * nayo money 股利工具 v33.0 - 視覺回歸穩定版
 * 更新重點：
 * 1. 廣告區塊修正：依照用戶要求，將「財富自由充電站」恢復為高對比度的原始樣式。
 * 2. 股數總額外顯：監測盤與投入明細在展開前即可見「總持股數」。
 * 3. 領息介面簡化：移除標的摺疊內的新增框，統一使用頂部全域區。
 * 4. 語法結構優化：徹底修復標籤不匹配與 onClick 語法錯誤。
 */
...
const CompactNumberInput = ({ value, onChange, className, placeholder, ...props }) => {
  ...
  return (
    <input
      ...
      step="any" 
      inputMode="decimal"
      className={`${className} text-sm font-black py-1 px-1.5 text-slate-800 outline-none transition-all border border-slate-200 rounded-lg focus:ring-2 ring-[#8B9D83]/30 bg-white shadow-sm`}
      value={value}
...
export default function App() {
  ...
  useEffect(() => {
    ...
    appleIcon.rel = 'apple-touch-icon'; appleIcon.href = iconData;
  ...
  const stats = useMemo(() => {
    ...
    fTx.forEach(t => {
      if(portfolio[t.symbol]) {
        ...
        if (parseFloat(t.cost) > 0) portfolio[t.symbol].lots.push({ id: t.id, date: t.date, cost: parseFloat(t.cost), shares: parseFloat(t.shares || 0), member: t.member });
      }
    });
    ...
    return { 
      totalDiv, totalMarketValue, totalCost, recovery: totalCost > 0 ? (totalDiv / totalCost) * 100 : 0, 
      ...
      monthly: Object.entries(monthlyData).sort((a, b) => b[0].localeCompare(a[0])), avgMonthly 
    };
  ...
  if (loading) return (
    <div className="min-h-screen bg-[#FDFBF7] flex flex-col items-center justify-center font-bold text-[#8B9D83]">
      ...
      <p className="text-xs uppercase tracking-widest italic opacity-60 text-center">Nayo Money Booting</p>
    </div>
  );
  ...
  return (
    <div className="min-h-screen bg-[#FDFBF7] text-slate-900 pb-20 font-sans select-none overflow-x-hidden text-left">
      <header className="bg-[#8B9D83] text-white py-1.5 px-4 sticky top-0 z-50 shadow-sm">
        <div className="max-w-7xl mx-auto w-full flex justify-between items-center text-white">
          ...
          <div className="flex items-center gap-2">
              <select value={filterMember} onChange={e => setFilterMember(e.target.value)} className="bg-white/20 text-white text-[10px] md:text-sm font-black border-none outline-none rounded-lg px-2.5 py-1 backdrop-blur-md cursor-pointer appearance-none shadow-sm">
      ...
      <main className="max-w-7xl mx-auto p-3 md:p-6 lg:p-12 space-y-4 lg:space-y-8 text-slate-900">
              ...
              <div className="space-y-5">
                    ...
                    stats.items.map(p => (
                      <div key={p.name} className="space-y-1.5 bg-slate-50/60 p-4 rounded-3xl border border-transparent hover:border-[#8B9D83]/20 transition shadow-sm mb-3 text-left">
                        ...
                        {expandedSymbol === p.name && (
                          <div className="mt-3 space-y-2 pt-2 border-t border-slate-200 animate-in slide-in-from-top-2 text-slate-900 text-left text-xs font-black text-slate-600">
                            {p.lots.map(lot => (
                              <div key={lot.id} className="flex justify-between">
                                <span><Clock size={12} className="inline mr-1 opacity-50"/>{lot.date} ({lot.member})</span>
                                <span>{lot.shares}股 | ${lot.cost.toLocaleString()} ({lot.progress.toFixed(2)}% 回本)</span>
                              </div>
                            ))}
                          </div>
                        )}
                ...
                <div className="bg-gradient-to-br from-[#8B9D83]/20 to-white rounded-[2rem] p-6 shadow-sm border border-[#8B9D83]/30 animate-in zoom-in text-slate-800 text-left">
                  <div className="flex items-center gap-3 mb-3 text-[#8B9D83]">
                    ...
                    <a href="https://nayomoney.com/" target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 bg-[#8B9D83] text-white px-5 py-2.5 rounded-2xl text-xs font-black shadow-lg shadow-[#8B9D83]/20 hover:scale-105 active:scale-95 transition-all text-white">
                      <ArrowUpRight size={16}/> 立即前往
                    </a>
                  </div>
                </div>
              </div>

              <div className="bg-white rounded-[2rem] p-5 md:p-8 shadow-sm border border-slate-100 h-fit text-slate-800 text-left">
                <div className="flex justify-between items-center border-b-2 pb-2 mb-4 text-slate-800">
                  <h3 className="font-black text-slate-800 text-sm md:text-base tracking-widest uppercase flex items-center gap-2 text-slate-800 text-left"><BarChart size={18} className="text-[#8B9D83]"/> 每月領息現金流</h3>
        ...
        {/* 投入紀錄區 */}
        {activeTab === 'invest' && (
          <div className="space-y-4 animate-in slide-in-from-right-4 duration-300 text-slate-900 text-left">
              ...
              <>
                ...
                <div className="bg-[#8B9D83]/10 p-4 rounded-[2rem] border border-[#8B9D83]/20 shadow-sm space-y-4 mb-6">
                   <div className="flex justify-between items-center px-1 text-slate-800">
                      <span className="text-xs font-black text-[#8B9D83] uppercase tracking-widest flex items-center gap-1 text-left text-[#8B9D83]"><PlusCircle size={14}/> 快速建立新投入</span>
                   ...
                   <div className="flex flex-wrap md:flex-nowrap gap-3 items-center">
                      <div className="flex flex-1 gap-2 items-center text-slate-800">
                        ...
                        <select value={txDraft.symbol} onChange={e => setTxDraft({...txDraft, symbol: e.target.value})} className="w-16 md:w-20 bg-white text-sm py-2 px-2 rounded-xl font-black border border-[#8B9D83]/20 text-slate-900 shadow-sm uppercase">
                          {symbols.map(s => <option key={s.id} value={s.name} className="text-slate-900">{s.name}</option>)}
                        </select>
                      </div>
                      <div className="flex flex-[3] gap-2 items-center text-slate-800">
                        ...
                        <button onClick={() => safeAddDoc('transactions', txDraft)} className="bg-[#8B9D83] text-white p-3 rounded-2xl shadow-lg active:scale-90 hover:bg-[#7A8C72] transition-all flex items-center justify-center border-2 border-white/20 text-white text-left"><Send size={18} /></button>
                      </div>
                   </div>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 text-slate-800 text-left">
                  {symbols.map(s => {
                    ...
                    const totalSharesForSym = txList.reduce((acc, curr) => (acc * 1000000 + parseFloat(curr.shares || 0) * 1000000) / 1000000, 0);
                        ...
                        {investExpanded === s.name && (
                          <div className="p-3 space-y-4 animate-in slide-in-from-top-2 text-slate-900 text-left">
                            {txList.sort((a,b) => b.date.localeCompare(a.date)).map(t => {
                              ...
                              return (
                                <div key={t.id} className={`p-4 rounded-[1.5rem] border-2 transition-all space-y-3 relative ${hasChanged ? 'border-amber-300 bg-amber-50/20 shadow-md scale-[1.01]' : 'border-slate-50 bg-white shadow-sm'} text-left text-slate-800`}>
                                  <div className="flex justify-between items-center text-slate-800 gap-2 text-left">
                                    ...
                                    <div className="flex items-center gap-1.5">
                                      {hasChanged && ( 
                                        <button onClick={() => handleUpdate('transactions', t.id, draft)} className="bg-emerald-600 text-white px-2 py-0.5 rounded-lg shadow-md flex items-center gap-1 animate-pulse font-black text-[10px] text-white text-left"><Check size={12}/> 存</button> 
                                      )}
                                      <button onClick={() => deleteDoc(doc(db, 'artifacts', currentAppId, 'users', user.uid, 'transactions', t.id))} className="text-slate-300 hover:text-red-500 p-1 transition-all text-left text-slate-800"><Trash2 size={18}/></button>
                                    </div>
                                  </div>
                                  <div className="flex gap-2 items-center text-slate-800 text-left">
                                    ...
                                    <div className="flex flex-1 gap-1.5 min-w-0 text-left">
                                      <CompactNumberInput value={draft.shares} onChange={v => setEditTx({...editTx, [t.id]: {...draft, shares: v}})} className="w-20 md:w-28 text-center shadow-inner" placeholder="股數" />
        ...
        {activeTab === 'dividends' && (
          <div className="space-y-4 animate-in slide-in-from-right-4 duration-300 text-slate-900 text-left text-slate-800">
            {!isReady ? ( <SetupGuide onGo={() => setActiveTab('masters')} /> ) : (
              <>
                <h2 className="font-black text-lg md:text-2xl text-slate-800 flex items-center gap-3 italic text-left text-slate-800"><DollarSign size={24} className="text-[#8B9D83]"/> 領息流水管理</h2>
                <div className="bg-[#8B9D83]/10 p-4 rounded-[2rem] border border-[#8B9D83]/20 shadow-sm space-y-4 mb-6 text-slate-800 text-left">
                   <div className="flex justify-between items-center px-1 text-left text-slate-800">
                      <span className="text-xs font-black text-[#8B9D83] uppercase tracking-widest flex items-center gap-1 text-left"><PlusCircle size={14}/> 全域快速新增領息</span>
                      <input type="date" value={divDraft.date} onChange={e => setDivDraft({...divDraft, date: e.target.value})} className="text-xs font-black bg-white rounded-lg px-3 py-1 border border-[#8B9D83]/20 outline-none shadow-sm cursor-pointer text-slate-800 text-left" />
                   </div>
                   <div className="flex flex-wrap md:flex-nowrap gap-3 items-center text-slate-800 text-left">
                      <div className="flex flex-[2] gap-2 text-slate-800 text-left">
                      ...
                      <div className="flex flex-1 gap-3 items-center text-slate-800 text-left">
                        <div className="flex-1 flex items-center bg-white border border-[#8B9D83]/20 rounded-xl px-3 py-1 shadow-inner text-slate-800 text-left">
                            ...
                            <CompactNumberInput value={divDraft.amount} onChange={v => setDivDraft({...divDraft, amount: v})} className="w-full text-right border-none shadow-none focus:ring-0 text-sm font-black text-slate-800 text-left" />
                        </div>
                        <button onClick={() => safeAddDoc('dividends', divDraft)} className="bg-[#8B9D83] text-white p-3 rounded-2xl shadow-lg active:scale-90 hover:bg-[#7A8C72] transition-all flex items-center justify-center border-2 border-white/20 text-white text-left"><Send size={18} /></button>
                      </div>
                   </div>
                </div>
                  ...
                  {symbols.map(s => {
                    ...
                    return (
                      <div key={s.name} className="bg-white rounded-[2rem] shadow-sm overflow-hidden border border-slate-100 h-fit text-slate-800 text-left text-slate-800 text-left">
                        <div className="p-4 bg-blue-50 border-b border-blue-100 flex justify-between items-center cursor-pointer hover:bg-blue-100 transition-colors text-slate-800 text-left text-slate-800 text-left" onClick={() => setDivExpanded(divExpanded === s.name ? null : s.name)}>
                          <span className="text-base font-black uppercase tracking-wide text-left text-slate-800 text-slate-800 text-left">{s.name} <span className="text-xs opacity-40 text-left">({divList.length})</span></span>
                          <div className="text-slate-400 text-left text-slate-800 text-left">{divExpanded === s.name ? <ChevronUp size={20}/> : <ChevronDown size={20}/>}</div>
                        </div>
                        {divExpanded === s.name && (
                          <div className="p-3 space-y-4 animate-in slide-in-from-top-2 text-slate-900 text-left text-slate-800 text-left">
                            <div className="border-t border-slate-100 pt-3 space-y-3 text-slate-800 text-left">
                              {divList.sort((a,b) => b.date.localeCompare(a.date)).map(d => {
                                ...
                                return (
                                  <div key={d.id} className={`p-4 rounded-[1.5rem] border-2 transition-all relative ${hasChanged ? 'border-amber-300 bg-amber-50/20 shadow-md scale-[1.01]' : 'border-slate-50 bg-white shadow-sm'} text-left text-slate-800 text-left`}>
                                    <div className="flex justify-between items-start mb-2 text-slate-800 text-left text-slate-800 text-left">
                                      <input type="date" value={draft.date} onChange={(e) => setEditDiv({...editDiv, [d.id]: {...draft, date: e.target.value}})} className="text-xs font-black outline-none bg-transparent text-slate-500 cursor-pointer text-left text-slate-800 text-left" />
                                      <div className="flex items-center gap-1 text-slate-800 text-left text-slate-800 text-left">
                                        {hasChanged && ( <button onClick={() => handleUpdate('dividends', d.id, draft)} className="bg-emerald-600 text-white px-2 py-0.5 rounded-lg shadow-md animate-pulse text-white text-left text-slate-800 text-left font-black text-[10px] text-white">存</button> )}
                                        <button onClick={() => deleteDoc(doc(db, 'artifacts', currentAppId, 'users', user.uid, 'dividends', d.id))} className="text-slate-300 hover:text-red-500 p-1 transition-all text-left text-slate-800 text-left">×</button>
                                      </div>
                                    </div>
                                    <div className="flex items-center gap-3 text-slate-800 text-left text-slate-800 text-left">
                                      ...
                                      <div className="flex-1 flex items-center bg-slate-50 border border-slate-100 rounded-xl px-3 py-1.5 text-slate-800 shadow-inner text-left text-slate-800 text-left">
                                          <span className="text-[10px] text-slate-400 font-black mr-1 text-left text-slate-800 text-left">NT$</span>
                                          <CompactNumberInput value={draft.amount} onChange={v => setEditDiv({...editDiv, [d.id]: {...draft, amount: v}})} className="bg-transparent text-right font-black text-slate-800 w-full outline-none text-sm border-none focus:ring-0 p-0 shadow-none text-left text-slate-800 text-left" />
                                      </div>
                                    </div>
                                  </div>
                                );
        ...
        {activeTab === 'masters' && (
          <div className="space-y-8 animate-in slide-in-from-bottom-4 duration-300 text-slate-900 pb-16 text-left text-slate-800">
            <div className="bg-white p-8 md:p-12 rounded-[3rem] shadow-sm border border-slate-50 text-left text-slate-800">
               <div className="space-y-4 text-left text-slate-800">
                 <h3 className="font-black text-xs md:text-sm text-slate-400 uppercase tracking-widest flex items-center gap-2 text-left mx-auto md:mx-0 text-slate-800 text-left"><Users size={20}/> 人員管理中心</h3>
                 <div className="flex gap-2 max-w-sm text-left text-slate-800 text-left">
                   <input placeholder="輸入姓名 (例如: 媽媽)" className="flex-1 py-3 px-5 text-base font-black text-slate-800 bg-slate-50 border border-slate-200 rounded-2xl outline-none focus:ring-2 ring-[#8B9D83]/20 shadow-inner text-slate-800 text-left text-slate-800 text-left" value={newMemName} onChange={e => setNewMemName(e.target.value)} />
                   <button onClick={async () => { if(newMemName.trim()) { await safeAddDoc('members', { name: newMemName.trim() }); setNewMemName(""); } }} className="bg-blue-600 text-white px-8 py-3 rounded-2xl font-black text-base shadow-xl active:scale-90 transition-all hover:bg-blue-700 text-white text-left text-white text-left">建立</button>
                 </div>
                 <div className="flex flex-wrap gap-2 pt-2 text-slate-800 text-left text-slate-800 text-left">
                   {members.map(m => (
                     <span key={m.id} className="bg-blue-50 text-sm font-black text-blue-800 px-6 py-3 rounded-2xl border border-blue-100 flex items-center gap-3 group shadow-sm text-slate-800 text-left text-slate-800 text-left">
                       {m.name}
                       <button onClick={() => deleteDoc(doc(db, 'artifacts', currentAppId, 'users', user.uid, 'members', m.id))} className="text-blue-300 hover:text-red-500 font-black px-1 transition-colors text-lg text-left text-slate-800 text-left">×</button>
                     </span>
                   ))}
                 </div>
               </div>

               <div className="border-t border-slate-100 pt-8 space-y-4 text-left text-slate-800 text-left text-slate-800 text-left">
                 <h3 className="font-black text-xs md:text-sm text-slate-400 uppercase tracking-widest flex items-center gap-2 text-left mx-auto md:mx-0 text-slate-800 text-left text-slate-800 text-left text-slate-800 text-left"><Globe size={20}/> 標的管理中心</h3>
                 <div className="flex gap-2 max-w-sm text-left text-slate-800 text-slate-800 text-left text-slate-800 text-left">
                   <input placeholder="標的代碼 (例如: 0050)" className="flex-1 uppercase py-3 px-5 text-base font-black text-slate-800 bg-slate-50 border border-slate-200 rounded-2xl outline-none focus:ring-2 ring-[#8B9D83]/20 shadow-inner text-slate-800 text-left text-slate-800 text-left" value={newSymName} onChange={e => setNewSymName(e.target.value.toUpperCase())} />
                   <button onClick={async () => { if(newSymName.trim()) { await safeAddDoc('symbols', { name: newSymName.trim(), currentPrice: 0 }); setNewSymName(""); } }} className="bg-[#8B9D83] text-white px-8 py-3 rounded-2xl font-black text-base shadow-xl active:scale-90 transition-all hover:bg-[#7A8C72] text-white text-left text-white text-left">新增</button>
                 </div>
                     ...
                     return (
                       <div key={s.id} className={`p-5 rounded-[2rem] border-2 transition-all ${hasChanged ? 'border-amber-300 bg-amber-50/20 shadow-md scale-[1.05]' : 'bg-white border-slate-100 shadow-sm hover:border-slate-300'}`}>
                         <div className="flex justify-between items-center mb-2 text-slate-800 text-left text-slate-800 text-left text-slate-800 text-left text-slate-800 text-left">
                            <span className="text-sm font-black uppercase text-slate-800 tracking-wider text-left text-slate-800 text-left">{s.name}</span>
                            <div className="flex items-center gap-1 text-slate-800 text-left text-slate-800 text-left">
                               ...
                               <button onClick={() => deleteDoc(doc(db, 'artifacts', currentAppId, 'users', user.uid, 'symbols', s.id))} className="text-slate-300 hover:text-red-500 transition-all p-1 text-[16px] font-black text-left text-slate-800 text-left">×</button>
                            </div>
                         </div>
                         <div className="flex items-center gap-2 pt-2 border-t border-slate-50 text-slate-800 text-left text-slate-800 text-left text-slate-800 text-left text-slate-800 text-left">
                           <span className="text-[10px] text-slate-400 font-black uppercase text-left text-slate-400 text-left">市價</span>
                           <CompactNumberInput value={draft.currentPrice} onChange={v => setEditSym({...editSym, [s.id]: {...draft, currentPrice: v}})} className="w-full text-center font-mono text-[#8B9D83] border-none shadow-none focus:ring-0 px-0 text-base shadow-none text-left text-slate-800 text-left" placeholder="0" />
                         </div>
                       </div>
                     );
                   })}
               ...
               <div className="border-t-4 border-[#8B9D83]/20 pt-12 pb-6 text-center mx-auto text-slate-800 text-left text-slate-800 text-center">
                 <div className="relative group mx-auto max-w-2xl bg-gradient-to-br from-[#8B9D83] to-[#7A8C72] rounded-[3rem] p-8 shadow-2xl overflow-hidden transition-all hover:scale-[1.02] active:scale-[0.98]">
                   ...
                   <div className="relative z-10 flex flex-col md:flex-row items-center gap-8 text-center md:text-left text-slate-800 text-center md:text-left">
                     ...
                     <div className="text-center md:text-left flex-1 text-slate-800 text-center md:text-left">
                       <h2 className="text-xl md:text-2xl font-black text-white mb-2 tracking-tight text-white text-center md:text-left text-white text-center md:text-left">探索更多財富自由的秘密</h2>
                 ...
                 <p className="text-[10px] text-slate-400 mt-6 font-black uppercase tracking-[0.3em] opacity-40 italic text-center">nayo money official recommendation</p>
               </div>
            </div>
          </div>
        )}
      ...
      <nav className="fixed bottom-0 left-0 right-0 z-50 pointer-events-none pb-safe text-left">
        <div className="max-w-7xl mx-auto w-full flex justify-center md:justify-end pointer-events-auto px-4 py-3 md:px-12 md:py-8 text-left">
          <div className="bg-white/95 backdrop-blur-md border border-slate-200 px-3 py-2 shadow-2xl flex justify-around items-center w-full md:w-fit md:rounded-full md:gap-10 animate-in slide-in-from-bottom-10 duration-700 text-slate-900 overflow-hidden min-h-[65px] md:min-h-[75px] text-left">
...
const StatCard = ({ title, value, sub, color }) => (
  <div className="bg-white p-6 md:p-10 rounded-[3rem] shadow-sm border border-[#D9C5B2]/10 active:scale-95 transition-transform text-center relative overflow-hidden group hover:shadow-xl mx-auto text-slate-800 text-center">
    ...
    <p className="text-[11px] md:text-xs font-bold text-slate-400 uppercase tracking-widest mb-3 leading-none text-center text-slate-400 text-center">{title}</p>
    <p className={`text-3xl md:text-5xl font-mono font-black tracking-tighter leading-none text-center text-slate-800 text-center`} style={{ color }}>{value}</p>
    <p className="text-[9px] md:text-[11px] text-slate-400 font-black italic tracking-wider uppercase opacity-80 mt-4 leading-none text-center text-slate-400 text-center">{sub}</p>
  </div>
);

const SetupGuide = ({ onGo }) => (
  <div className="bg-white p-14 rounded-[4rem] text-center space-y-6 shadow-2xl border border-amber-50 animate-in zoom-in max-w-xl mx-auto mt-12 text-slate-800 mx-auto text-left text-center text-slate-800 text-center text-slate-800 text-center">
    <div className="bg-amber-50 w-24 h-24 rounded-full flex items-center justify-center mx-auto text-amber-500 shadow-inner mb-3 text-amber-500 text-center text-amber-500 text-center"><AlertCircle size={56} /></div>
    <div className="space-y-2 text-center mx-auto text-slate-800 text-center text-slate-800 text-center">
      <h3 className="text-3xl font-black tracking-tight text-center leading-tight text-slate-800 text-center text-slate-800 text-center">尚未完成初始化</h3>
      <p className="text-base text-slate-500 font-bold px-8 leading-relaxed text-center text-slate-800 text-center text-slate-800 text-center">請前往「管理」分頁建立人員與標的。</p>
    </div>
    <button onClick={onGo} className="bg-blue-600 text-white w-full max-w-xs py-5 rounded-[2rem] font-black text-xl shadow-xl active:scale-95 transition-all mx-auto tracking-widest uppercase flex items-center justify-center gap-3 text-center text-white text-center text-white text-center">立即前往 <ArrowRight size={24}/></button>
  </div>
);
...
const NavBtn = ({ active, onClick, icon, label }) => (
  <button onClick={onClick} className={`flex flex-col items-center justify-center gap-0.5 transition-all duration-300 px-2 min-w-[65px] flex-shrink-0 ${active ? 'text-[#8B9D83] scale-110' : 'text-slate-400 hover:text-slate-600'}`}>
    ...
    <span className={`text-[10px] md:text-xs font-black tracking-widest leading-none ${active ? 'text-[#8B9D83]' : 'text-slate-50'}`}>{label}</span>
  </button>
);
